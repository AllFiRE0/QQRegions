...
| `menus/*.yml` | все GUI: info, flags, players, playerconfirm, market, marketconfirm, flagshop, blocks, myflags, help и др. |
...
показывает флаг в меню флагов **и в магазине флагов** и разрешает его
менять/покупать. Право `qqregions.admin` видит/меняет/покупает всё.
Простой вариант — выдать привилегию `qqregions.flags.use.*` (например в
LuckPerms группе игроков): тогда игрок получит доступ ко всем флагам.
Legacy-право `qqregions.flags.<флаг>` также принимается (для совместимости).

**Группы-шаблоны (`config.yml` → `flag-groups`)** — выдача флагов «пачкой»:
игроку с правом `qqregions.flags.group.<имя>` автоматически открываются все
флаги из списка этой группы (видеть/менять/покупать). Несколько групп
суммируются; `["*"]` в списке = все флаги. Работает вместе с отдельными
`qqregions.flags.use.<флаг>`.

> Пример с группами: группе «новички» выдайте `qqregions.flags.group.newbie`
> (в конфиге — `pvp`, `build`), а группе «старожилы» — `qqregions.flags.group.veteran`
> (широкий список) — у новичков в магазине и меню флагов будет виден только
> их набор, у старожилов — свой; общее право можно докинуть точечно через
> `qqregions.flags.use.<флаг>`.
...
PlaceholderAPI. Отключить полностью можно через `placeholders.enabled`
в `config.yml`; разделители списков (`owners-separator`,
`members-separator`, `owned-separator`, `membered-separator`) — там же,
в секции `placeholders`.
...
| `%qqregions_selection_pos1_x/y/z%` | координаты первой точки (как поставлена) |
| `%qqregions_selection_pos2_x/y/z%` | координаты второй точки |
...
| `%qqregions_region_flags%` | установленные флаги региона: `флаг:значение, ...` |
| `%qqregions_region_flag_<флаг>%` | значение конкретного флага (пусто, если не задан) |
| `%qqregions_region_owners%` | имена владельцев текущего региона |
| `%qqregions_region_members%` | имена участников (без владельцев) текущего региона |
| `%qqregions_region_owner_is%` | `yes`/`no` — игрок владелец текущего региона |
| `%qqregions_region_member_is%` | `yes`/`no` — игрок участник (владелец тоже) текущего региона |
| `%qqregions_region_role%` | роль в текущем регионе: `OWNER` / `MEMBER` / `NONE` |
...
| `%qqregions_region_rent_time_<мир:регион>%` | срок аренды региона |
| `%qqregions_player_owned_regions%` | регионы игрока, где он владелец (админ — все), через запятую |
| `%qqregions_player_owned_count%` | их количество |
| `%qqregions_player_membered_regions%` | регионы, где игрок участник (не владелец), через запятую |
| `%qqregions_player_membered_count%` | их количество |
...
Горизонтальное (2D) расстояние от точки игрока до границы региона;
0, если игрок стоит внутри. Регионы из `restrictions.banned-regions`
в списках и подсчётах не учитываются.

| Заполнитель | Значение |
|---|---|
| `%qqregions_nearby_region%` | id ближайшего региона (пусто, если нет) |
| `%qqregions_nearby_region_distance%` | расстояние до ближайшего региона в блоках |
| `%qqregions_nearby_region_count%` | число регионов в радиусе 100 блоков |
| `%qqregions_nearby_region_count_<радиус>%` | то же с явным радиусом |
...
| `@select` | включить интерактивный выбор («Создать территорию» из главного меню) |
...
региона лор (мир/тип/игроки/площадь/расстояние), клик — info.
Сортировка `@rpsort` циклирует: близкие → дальние → A-Z → Z-A →
по людям (возр./убыв.) → по площади (возр./убыв.); текущий режим
подсвечен зелёным в лоре кнопки «Сортировка».

Кнопка **«Создать территорию»** (слот 31 главного меню, `@select`)
включает интерактивный выбор — как `/region select` без аргументов:
меню закрывается, в хотбар выкладываются кнопки «Точка 1» / «Точка 2»
(клик — поставить точку, колесо мыши — двигать активную), «Создать»
(ввод названия), «Сбросить» и «Отмена».
...
Флаг игроку показывается только при праве на него: `qqregions.flags.use.<флаг>`
(через LuckPerms или игроку по отдельности), legacy `qqregions.flags.<флаг>`
или группе-шаблону `qqregions.flags.group.<имя>` из `config.yml` →
`flag-groups`. Админ/оп видят всё. Купленные в магазине флаги видны всегда.
...
первыми, затем оффлайн, без себя; головы со скинами игроков. На каждой
голове видны выбранная группа добавления, баланс (Vault), число регионов
(владелец+участник) и максимум игрока, клан (JustTeams).
ЛКМ — добавить в выбранную группу (подтверждение), ПКМ — убрать
(подтверждение), Шифт+ЛКМ — сменить группу добавления. Сортировка кнопкой
«Сортировка» (слот 4): A-Z/Z-A/по балансу/по числу регионов (возр./убыв.)/
по дистанции (дальше/ближе). Последнего владельца снять нельзя.
Подтверждение добавления/удаления — меню playerconfirm.yml.
...
Продаваемые флаги (все из реестра WorldGuard — включая флаги из любых других
плагинов, например WGEFP, кроме `flags-menu.whitelist` и
`flags-menu.shop-ignore`) с ценами из `shop.yml`. Игрок **видит и может
купить** флаг только при праве на него: `qqregions.flags.use.<флаг>`
(отдельно или через LuckPerms группу) **или** группу-шаблон
`qqregions.flags.group.<имя>` из `config.yml` → `flag-groups` — пока права
нет, флага в магазине нет и купить его нельзя; после выдачи — появляется.
Legacy-право `qqregions.flags.<флаг>` также принимается. Админ/оп видят и
покупают всё. Поиск — по имени или переводу.
...
| `flag-groups` | группы-шаблоны прав на флаги: право `qqregions.flags.group.<имя>` открывает сразу весь список флагов (см. раздел 4) |
...
| `placeholders` | интеграция PlaceholderAPI: `enabled` (вкл/выкл расширение `%qqregions_*%`), `owners-separator` / `members-separator` / `owned-separator` / `membered-separator` — разделители списков владельцев, участников и регионов игрока |

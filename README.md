...
```text
/region flags <регион>     — меню флагов (ЛКМ — вкл/выкл, ПКМ — группа)
...
|---|---|
| Меню info | `{owners}` `{members}` `{type}` `{area}` `{volume}` `{priority}` `{status}` |
...
(с `lore`, командой клика, опциональным `permission`), динамические
...
| `@flag:<имя>:<allow\|deny>` | переключить флаг |
...
Открывается `/region info`. У владельца видны: название, тип, площадь,
владельцы/участники, кнопки **Флаги**, **Игроки**, **Рынок**,
**Телепорт** (право `qqregions.admin`), **Подсветка**, **Удалить**,
**Помощь**. У постороннего — только базовая информация + кнопка
**Рейд** (`role-required: other`, право `qqregions.raid`).
...
(`flags-names`), значения — из `replace.yml`. ЛКМ — переключить значение,
ПКМ — сменить **группу** (all/members/owners/nonmembers/nonowners).
...
удалить владельцев / Все». Управление — только владельцы и админы.
...
| `highlight` | подсветка регионов: тип, `show-seconds`, кулдаун, `hide-on-exit`, `territory` (забор + `ignore-blocks`), `particles` |

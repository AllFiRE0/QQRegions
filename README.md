...
| `menu-update` | автообновление меню: `ticks` (глобальный интервал перерисовки открытых меню, по умолчанию 20) и `debounce-after-click` (анти-автокликер: клик по кнопке откладывает следующую перерисовку на `update_interval` меню) |
...
открытое меню (лore/цены/флаги обновляются на лету); `0` = выключить
автообновление этого меню. Глобальный дефолт — `menu-update.ticks` в
config.yml. Анти-автокликер (`menu-update.debounce-after-click: true`)
молча откладывает перерисовку открытого меню после клика по кнопке на
`update_interval` тиков — автокликеры не грузят сервер; навигация и
действия кнопок мгновенные. Подробности — в `menus/MENU_EDITOR.md` §8.
